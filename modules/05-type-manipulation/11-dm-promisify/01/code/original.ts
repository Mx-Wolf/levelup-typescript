...
export const createGameBoard = (): GameBoardView => {
  ...
  {return {
    neutralizeEvil: (done) => evil.addEventListener('click', done, { once: true }),
    encourageMate: (done) => mate.addEventListener('click', done, { once: true }),
    disarmDevice: (done) => timer.addEventListener('click', done, { once: true }),
  };}
};
